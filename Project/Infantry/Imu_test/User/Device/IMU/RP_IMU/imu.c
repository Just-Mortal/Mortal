#include "imu.h"
  ...
  #define IMU_POSE_ANGLE  90.f
#endif 

#if !defined  (IMU_POSE_AX)
  #define IMU_POSE_AX    	0.f
#endif 
...
float yaw_acc, roll_acc, pitch_acc;
float yaw_w,	roll_w,	pitch_w;// 世界坐标速度
float yaw_gyr		= 0.f;				// 上一次的状态
float roll_gyr	= 0.f;
float pitch_gyr	=	0.f;
float yaw_pre		= 0.f;				// 下一次的估计
float roll_pre	= 0.f;
float pitch_pre	=	0.f;
float roll_last;
float pitch_last;
...
void imu_updata(struct imu_struct *self)
{
	...
	acc_z = imu.data.acc_gyr.acc_z;
	gyr_x = imu.data.acc_gyr.gyr_x;
	gyr_y = imu.data.acc_gyr.gyr_y;
	gyr_z = imu.data.acc_gyr.gyr_z;
	
	// 加速度姿态解算
	arm_atan2_f32(acc_y, acc_z, &roll_acc);
	arm_atan2_f32(acc_x, sqrt(pow(acc_y, 2)+pow(acc_z, 2)), &pitch_acc);
	pitch_acc	= -pitch_acc;
	roll_acc	*= 57.295779513f;
	pitch_acc	*= 57.295779513f;
	
	// 解算角速度
	float	p = pitch_gyr * 0.01745329252f;
	float r	=	roll_gyr	* 0.01745329252f;
	roll_w = gyr_x +	gyr_y*sin(p)*sin(r)/cos(p)	+ gyr_z*cos(r)*sin(p)/cos(p);
	pitch_w=					gyr_y*cos(r)								-	gyr_z*sin(r);
	yaw_w	 =					gyr_y*sin(r)/cos(p)					+ gyr_z*cos(r)/cos(p);
	
	// 状态估计
	pitch_last= pitch_pre;
	roll_last	= roll_pre;
	pitch_pre	=	A[0][0]*pitch_gyr + B[0][0]*pitch_w*57.295779513f;
	roll_pre	=	A[1][1]*roll_gyr	+ B[1][1]*roll_w*57.295779513f;
	
	// 协方差矩阵计算
	float P_pre[2][2];
	P_pre[0][0]	=	P[0][0] + Q[0][0];
	P_pre[0][1]	=	P[0][1] + Q[0][1];
	P_pre[1][0]	=	P[1][0] + Q[1][0];
	P_pre[1][1]	=	P[1][1] + Q[1][1];
	
	// 计算卡尔曼系数
	float K11 = P_pre[0][0] / (P_pre[0][0] + R[0][0]);
	float K22 = P_pre[1][1] / (P_pre[1][1] + R[1][1]);
	
	// 最优估计
	roll_gyr = roll_last + K11 * (roll_acc - roll_last);
	pitch_gyr= pitch_last+ K22 * (pitch_acc - pitch_last);
	
	// 更新协方差
	P[0][0] = (1.f - K11)*P_pre[0][0];
	P[1][1] = (1.f - K11)*P_pre[1][1];
...
void imu_spi_get_data(struct imu_struct *self)
{
	...
	self->data.acc_gyr.gyr_x = lsb_to_dps(self->data.rawData.gyr_x,2000,imu.bmi->dev->resolution) * 0.0174532925f;  //转弧度
	self->data.acc_gyr.gyr_y = lsb_to_dps(self->data.rawData.gyr_y,2000,imu.bmi->dev->resolution) * 0.0174532925f;
	self->data.acc_gyr.gyr_z = lsb_to_dps(self->data.rawData.gyr_z,2000,imu.bmi->dev->resolution) * 0.0174532925f;
